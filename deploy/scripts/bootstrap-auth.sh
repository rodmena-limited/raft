...
# The container healthcheck runs `etcdctl endpoint health` INSIDE the image,
# which has no shell -- so it cannot expand a variable in a CMD string. etcdctl
# reads ETCDCTL_USER directly from the environment, so the compose healthcheck
# gets this file instead. It holds only the app credential, never root, so the
# admin password is not injected into three long-running containers.
#
# Without this, `endpoint health` runs unauthenticated once auth is enabled and
# reports every member unhealthy forever -- a check that can never go green.
HEALTH_FILE="$(dirname "$CRED_FILE")/healthcheck.env"
umask 077
cat > "$HEALTH_FILE" <<EOF
# Generated by bootstrap-auth.sh. Consumed by docker-compose healthchecks.
ETCDCTL_USER=app:${ETCD_APP_PASSWORD}
ETCDCTL_ENDPOINTS=http://127.0.0.1:2379
EOF
# `docker compose` reads env_file as the invoking user, so this one file must be
# readable by the docker group -- which is already root-equivalent (a docker
# group member can bind-mount any host path into a container), so this grants
# nothing new. credentials.env stays 0600 root-only: the ADMIN password is never
# widened, and never enters a container.
chgrp docker "$(dirname "$CRED_FILE")" "$HEALTH_FILE" 2>/dev/null || true
chmod 0750 "$(dirname "$CRED_FILE")"
chmod 0640 "$HEALTH_FILE"
echo "wrote healthcheck identity to $HEALTH_FILE (root:docker 0640)"
